...
def action(sno, pwd):
    options = Options()
    options.add_argument('--no-sandbox')  # 让Chrome在root权限运行
    ...
    service = Service('/home/xxl/Python/chromedriver')  # 传入chromedriver路径，用绝对路径，方便使用crontab创建定时任务
...
if __name__ == '__main__':
    ...
    system("killall chrome")  # 杀死所有chrome进程(防止chrome进程在后台过多占用资源)