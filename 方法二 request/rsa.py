...
def sBoxPermute(expandByte):
    ...
    for m in range(8):
        ...
        if m == 0:
            binary = bin_test(s1[i][j])
        elif m == 1:
            binary = bin_test(s2[i][j])
        elif m == 2:
            binary = bin_test(s3[i][j])
        elif m == 3:
            binary = bin_test(s4[i][j])
        elif m == 4:
            binary = bin_test(s5[i][j])
        elif m == 5:
            binary = bin_test(s6[i][j])
        elif m == 6:
            binary = bin_test(s7[i][j])
        elif m == 7:
            binary = bin_test(s8[i][j])
...
def strEnc(data):
    ...
    secondKey = '2'
    thirdKey = '3'
    firstKeyBt = str_to_16bin(firstKey)
    secondKeyBt = str_to_16bin(secondKey)
    thirdKeyBt = str_to_16bin(thirdKey)
    ...
    for i in range(irator):
        ...
        tempBtyte = str_to_16bin(tempData)
        ...
        encByte = tempBt
        encData += bin2hex(encByte)
    if remainder > 0:
        remainderData = data[irator * 4 + 0:leng]
        tempByte = []
        if leng <= 4:
            tempByte = str_to_16bin(remainderData)
        else:
            tempByte = str_to_16bin(remainderData[0:4])
        # encByte=[]
